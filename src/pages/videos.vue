<template>
  <div class="main">
    <div class="section">
      <div class="title">
        <div class="t-left" v-if="queryInfo.type === 'movie'">电影</div>
        <div class="t-left" v-if="queryInfo.type === 'tv'">电视剧</div>
        <div class="t-left" v-if="queryInfo.type === 'cartoon'">动漫</div>
        <div class="t-center">
          <div class="t-search">
            <el-input placeholder="请输入内容" v-model="queryInfo.search" class="input">
              <el-button type="primary" icon="el-icon-search" slot="append" @click="loadMovieList" >搜索</el-button>
            </el-input>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      loadMovieList() {
        ...
        this.axios.get('/api/video/findPage?'+this.qs.stringify(this.queryInfo))
          .then(res => {
            var r = res.data;
            if(r.code){
